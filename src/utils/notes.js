...
export function addNote(db, dealerId, { text, user, profile }) {
  return addDoc(collection(db, 'dealers', dealerId, 'notes'), {
    text,
    byUid: user.uid,
...
export function editNote(db, dealerId, noteId, text) {
  return updateDoc(doc(db, 'dealers', dealerId, 'notes', noteId), { text, editedAt: serverTimestamp() });
}

export function removeNote(db, dealerId, noteId) {
  return deleteDoc(doc(db, 'dealers', dealerId, 'notes', noteId));
}
